...
Security integration finding: Pi's `needs_tools: false` is a routing hint, not an execution prohibition; its loop can still present and invoke available tools. The narrow live composer must not be described as an answer-only safety boundary. Before configured live effects are enabled, implement server-enforced answer-only execution or fresh operation-bound verification for dispatch-capable submissions/resumes and owner decisions. Cookie/CSRF authentication alone does not satisfy the owner's fresh-verification requirement. QA uses no provider or gate connections.

Next execution contract: reserve a durable turn request identity before automatic fork summarization; bind effective session, input message, turn and optional task in one transaction; associate exact tool/intermediate/final messages without guessing older history. Final-message append and turn completion must commit together. Unknown effects reconcile their existing ToolGate action ID. Task-bound automatic forks should fail before dispatch until task lineage is supported. Pending input and request-comparison hashes must participate in forgetting. Identical retries inspect/return the saved submission rather than repeat model/tool work.

## Current next action

P2 Activity, P3 specialist CRUD, P4 Teams/Templates, P5 Projects, P6 context controls, P7 helper-model configuration and P8 native artifacts/canvas have verified preview increments. The production entry now authenticates through Pi and exposes real conversation transport and durable Tasks/Runs/Events without loading fixtures. Next: operation-bound fresh password verification, a server-enforced unlock deadline, durable turn request identities and exact input/output associations. Do not infer message-to-turn links from timestamps. Durable full-workspace state, real task/team dispatch and helper-model execution remain integration work; preview changes still reset on reload.
