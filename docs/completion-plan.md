...
| 2026-09-20 | Teams and Templates | 96c941ae | Owner-authored template drafts, immutable published versions, reviewed instantiation, scoped team roles/handoffs/budgets and immutable prepared snapshots. Current published tool catalogue shared with Agents; concurrent deletion and selection serialized. Browser verified template-to-agent creation, retained team draft, searchable assignment, team save/preparation, mobile inspector and budget rejection. Domain, agent, task, navigation checks, lint/type/build passed. Team execution remains unwired. |
| 2026-09-20 | Project workspace | 6aec27f9 | Owner-authored grouping and instructions; searchable conversation/task/file references; source inspector; scoped metadata search and privacy-aware context preview; archive/restore and empty-project removal. Domain and real fixture integration tests cover revision conflicts, private/missing/moved origins and retained source records. Browser exercised create/save/link/context/archive/restore/unlink, desktop light and mobile dark; no overflow or console errors. Build/lint pass. Content ingestion and context injection remain explicitly unwired. |

| 2026-09-20 | Native artifact workspace | 384844a9 | Source-bound copies and citations, immutable editable versions, safe export, task links and shared chat canvas. Browser verified edit/restore/history links, light desktop and 390px mobile, chart preview and call-to-library mini view. Domain/integration/renderer/call/navigation checks, lint and build passed. See artifact-workspace.md for remaining P8 scope. |
| 2026-09-20 | Same-origin gateway assets (Pi repository) | 265134d6, 13d99366 | Optional bounded dist snapshot, reserved API paths and strict separate UI CSP; 51 gateway and 2 TLS tests, Ruff, actual 161-file dist inventory. Follow-up permits the verified bundled KaTeX data font with 45 gateway tests. Local browser QA only; no deployment. Gateway frontend disables external fonts/analytics. |
| 2026-09-20 | Authenticated conversation gateway | 5f5d84a7 | Explicit fixture/live modes, same-origin cookie/CSRF auth, fixture-free module graph, real session/message DTOs, non-replayed mutations and uncertainty reconciliation. Focused gateway/theme checks, lint, TypeScript and build pass. Browser checked actual HTTPS authentication with simulated Pi: create/send/history/reload/logout, lost acknowledgement, desktop dark/mobile light and no external font/analytics loading. See gateway-workspace.md for exact scope and remaining security/runtime work. |
| 2026-09-20 | Durable task ledger (Pi repository) | 11eeb59 | SQLite task metadata, atomic revision checks, creation request identities, fixed-source run links and append-only content-free events. Fifteen ledger tests, broader Store/Loop/Forgetting/Gateway regression checks and independent shared-run/concurrency review passed. Physical forgetting includes task text and comparison hashes. No executor, inferred output associations or deployed service claim. |
| 2026-09-20 | Live Activity workspace | a4143bf1 | Authenticated Tasks/Runs/Events, linked-source navigation, retained drafts, reviewed completion, archive/restore, strict DTOs and explicit uncertain-write recovery. Real SQLite browser QA covered lifecycle/restart, concurrent edits, a dropped acknowledgement recovered with one creation request, desktop light/dark and narrow dark screenshots. Source-switch race repaired. Gateway checks and production build pass. See live-activity.md. |
| 2026-09-20 | Verified writes and durable submissions (Pi repository) | f42c1a7, 3be2f22 | One-use operation-bound password proofs, server-enforced verified deadline, durable submission reservation, atomic input/turn/task binding and exact message provenance. Focused suites, Ruff and independent replay/race/history review passed. |
| 2026-09-20 | Protected writes and submission recovery | d02aad34 | Fresh verification UI, deadline locking, retained request identities, reload discovery, saved preparation input and exact-message navigation. Browser checked cancelled/wrong-password attempts, mobile task verification, deadline lock and a lost committed turn acknowledgement recovered with exactly one stub-provider call. Gateway checks and production build pass. See gateway-verification-recovery.md. |
| 2026-09-20 | Live task dispatch | This increment | Reviewed saved objective through the existing verified conversation pipeline; fixed task/revision binding, separate chat drafts, conflict and lost-ack recovery, immediate privacy masking. Real SQLite browser QA verified stale revision rejection, one linked stub-provider run, unchanged ordinary draft and no automatic completion. See live-task-dispatch.md. |
...
Security integration finding: Pi's `needs_tools: false` is a routing hint, not an execution prohibition; its loop can still present and invoke available tools. The narrow live composer must not be described as an answer-only safety boundary. Fresh operation-bound verification now protects gateway runtime writes and owner decisions. This does not replace ToolGate effect grants or action reconciliation. QA uses a labeled stub provider and no external gate connections.

Implemented execution contract: reserve a durable turn request identity before automatic fork summarization; bind effective session, input message, turn and optional task in one transaction; associate exact tool/intermediate/final messages without guessing older history. Final-message append and turn completion commit together. Unknown effects reconcile their existing ToolGate action ID. Task-bound automatic forks fail before dispatch until task lineage is supported. Pending input and request-comparison hashes participate in forgetting. Identical retries inspect/return the saved submission rather than repeat model/tool work.
