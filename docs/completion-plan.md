...
| 2026-09-20 | Native artifact workspace | this increment | Source-bound copies and citations, immutable editable versions, safe export, task links and shared chat canvas. Browser verified edit/restore/history links, light desktop and 390px mobile, chart preview and call-to-library mini view. Domain/integration/renderer/call/navigation checks, lint and build passed. See artifact-workspace.md for remaining P8 scope. |
| 2026-09-20 | Same-origin gateway assets (Pi repository) | 265134d6 | Optional bounded dist snapshot, reserved API paths and strict separate UI CSP; 51 gateway and 2 TLS tests, Ruff, actual 161-file dist inventory. No service launch or deployment. External Google Fonts disabled by CSP; frontend follow-up pending. |
...
P2 Activity, P3 specialist CRUD, P4 Teams/Templates, P5 Projects, P6 context controls and P7 helper-model configuration have verified preview increments. P8 native output retention/versioning and shared canvas now have a verified preview increment. Continue the authenticated backend path; isolated interactive applications, uploaded media and model-driven canvas updates remain P8/P11 work. Server persistence, real task/team execution and helper-model dispatch remain integration work; fixture changes still reset on reload.
