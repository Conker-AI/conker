...
| 2026-09-20 | Authenticated conversation gateway | this increment | Explicit fixture/live modes, same-origin cookie/CSRF auth, fixture-free module graph, real session/message DTOs, non-replayed mutations and uncertainty reconciliation. Focused gateway/theme checks, lint, TypeScript and build pass. Browser checked actual HTTPS authentication with simulated Pi: create/send/history/reload/logout, lost acknowledgement, desktop dark/mobile light and no external font/analytics loading. See gateway-workspace.md for exact scope and remaining security/runtime work. |
...
P2 Activity, P3 specialist CRUD, P4 Teams/Templates, P5 Projects, P6 context controls, P7 helper-model configuration and P8 native artifacts/canvas have verified preview increments. The production entry now authenticates through Pi and exposes a narrow real conversation transport without loading fixtures. Next: durable Pi task metadata, actual turn/run references and content-free events, with revision conflicts, explicit mutation reconciliation and forgetting coverage. Do not add an executor or infer message-to-turn links from timestamps. Fresh privileged verification, user-presence idle locking, durable full-workspace state, real task/team dispatch and helper-model execution remain integration work; preview changes still reset on reload.
