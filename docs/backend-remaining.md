...
| P10 tools/jobs | Published pinned execution, nested workflows, deterministic schedule and receipts already exist. | Pi `ba49b98` binds an existing actor/root-scoped ToolGate budget to a held run; ToolGate `8c0780b` adds scoped metadata reads. Recurring delegated budget provisioning and combined service contract checks remain. Arbitrary code execution is explicitly deferred by P10's JSON-source rule. |
