...
async function main() {
  ...
  await check('Edits invalidate scenario cards and update legacy source text', async () => {
    ...
    await assert.rejects(client.updateMessage('week', 'intent', { text: 'A corrected question.' }), /Fork/)
    const branch = await client.forkConversation('week', 'intent')
    const user = await client.updateMessage(branch.id, 'intent', { text: 'A corrected question.' })
    assert.equal(user.edited, true)
    assert.equal((await client.load()).messages[branch.id][0].text, user.text)
    assert.notEqual((await client.load()).threads.week.messages[0].text, user.text)
