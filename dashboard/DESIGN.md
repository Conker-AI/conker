...
The header owns Call, the model/provider choice, Incognito, settings, optional browser fullscreen and minimize. Call settings use centered `TaskDialogContent` for Focus/Character, devices, language, privacy explanations and session details. Browser fullscreen failure must leave the expanded call usable.
