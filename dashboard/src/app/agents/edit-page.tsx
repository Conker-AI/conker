...
function AgentEditor({ agent }: { agent: Agent }) {
  ...
  return <form onSubmit={event => void submit(event)} className="space-y-6">
    <fieldset disabled={pending || !!agent.archivedAt} className="grid min-w-0 items-start gap-6 lg:grid-cols-12">
      ...
      <ReferenceSection title="Tool allowlist"><p className="text-sm text-muted-foreground">Tools this agent may request. This configuration grants no execution authority; approvals and limits remain separate.</p><div className="space-y-3">{tools.map(tool => <div key={tool.id} className="flex items-start gap-3"><Checkbox id={`agent-tool-${tool.id}`} checked={value.toolIds.includes(tool.id)} onCheckedChange={checked => setValue({ ...value, toolIds: select(value.toolIds, tool.id, checked === true) })} /><Label htmlFor={`agent-tool-${tool.id}`} className="block"><span>{tool.name}</span><span className="mt-1 block text-xs font-normal text-muted-foreground">{tool.purpose}</span></Label></div>)}</div>{!tools.length && <p>No tools available.</p>}</ReferenceSection>
      <ReferenceSection title="Memory context"><div className="space-y-2"><Label htmlFor="agent-memory">Memory scope</Label><Select value={value.memory.scope} onValueChange={scope => setValue({ ...value, memory: { scope: scope as AgentInput["memory"]["scope"], memoryIds: scope === "selected" ? value.memory.memoryIds : [] } })}><SelectTrigger id="agent-memory"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">No memory</SelectItem><SelectItem value="conversation">Conversation context</SelectItem><SelectItem value="selected">Selected memories</SelectItem></SelectContent></Select></div>{value.memory.scope === "selected" && <div className="space-y-3">{memories.map(memory => <div key={memory.id} className="flex items-start gap-3"><Checkbox id={`agent-memory-${memory.id}`} checked={value.memory.memoryIds.includes(memory.id)} onCheckedChange={checked => setValue({ ...value, memory: { ...value.memory, memoryIds: select(value.memory.memoryIds, memory.id, checked === true) } })} /><Label htmlFor={`agent-memory-${memory.id}`} className="block leading-5">{memory.title || memory.text}</Label></div>)}{!memories.length && <p>No memory records available. Choose another scope.</p>}</div>}<p className="text-xs text-muted-foreground">References for new conversations only. Memory retrieval and writes are not connected.</p></ReferenceSection>
      </div>
    </fieldset>
