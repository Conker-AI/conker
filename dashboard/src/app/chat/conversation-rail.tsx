...
function RailContent({ session, children }: { session: Session; children?: ReactNode }) {
  ...
  return <div className="space-y-5 px-4 pb-6 text-sm">
    ...
    <section className="space-y-2 border-t pt-4" aria-label="Memory scope"><h3 className="font-medium">Memory scope</h3><Badge variant="outline">{conversation.incognito ? "Incognito on" : "Incognito off"}</Badge><p className="text-xs leading-5 text-muted-foreground">{conversation.incognito ? "Memory reads and writes are off for this conversation in the preview." : `Read scope: ${conversation.memory.scope}. Memory writes are not connected.`}</p>{!conversation.incognito && conversation.memory.sources.map(source => <p key={source.id} className="text-xs">{source.label}</p>)}</section>
