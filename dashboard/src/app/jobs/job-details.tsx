...
export function JobDetails({ job, agentName, controls }: { job: Job; agentName: string; controls: JobControls }) {
  return <>
    ...
    <div className="min-h-0 flex-1 space-y-6 overflow-y-auto p-5">
      <section aria-label="Job configuration" className="space-y-4">
      ...
      <section className="space-y-3 border-t pt-5"><h3 className="text-sm font-medium">Instructions</h3><p className="text-sm leading-6 whitespace-pre-wrap break-words text-muted-foreground">{job.instructions}</p></section>
      <section className="space-y-3 border-t pt-5">
        <h3 className="flex items-center justify-between text-sm font-medium">Run history<span className="text-xs font-normal text-muted-foreground">{job.history.length} {job.history.length === 1 ? "run" : "runs"}</span></h3>
