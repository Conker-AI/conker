...
const routes: Record<GatewayMethod, RegExp[]> = {
  GET: [ /^\/api\/control\/pi\/models\/configuration$/, /^\/api\/control\/pi\/memory\/objects$/, new RegExp(`^/api/control/pi/memory/objects/[a-z]+/${ids}$`),
  ...
  POST: [ /^\/api\/control\/pi\/models\/configuration$/, /^\/auth\/(login|logout|verify|revoke-all)$/, new RegExp(`^/auth/sessions/${ids}/revoke$`),
