...
export type Selection =
  ...
  | { kind: "object"; resource: string; id: string };
...
export type Command = {
  ...
  icon: LucideIcon;
  capability?: string;
  available: (context: CommandContext) => boolean | string;
...
export type ViewContribution = {
  id: string;
  slot: "message.annotations" | "composer.tools" | "inspector.tabs";
  label: string;
  capability?: string;
  applies: (context: CommandContext) => boolean;
  Component: ComponentType<{ context: CommandContext }>;
};
...
export type NavigationContribution = {
  ...
  to: string;
  icon: LucideIcon;
  group: "Daily loop" | "Reference" | "Control";
};
export type Feature = {
  id: string;
  shipped: boolean;
...
export function createRegistry(features: Feature[]) {
  ...
  for (const placement of placements)
    if (!commands.some((command) => command.id === placement.commandId))
      throw new Error(`Unknown command ${placement.commandId}.`);
  for (const item of navigation)
    if (!routes.some((route) => route.id === item.routeId))
      throw new Error(`Unknown route ${item.routeId}.`);
  return {
    commands,
    placements,
    views,
    routes,
    navigation,
    features: features.map(({ id, shipped }) => ({ id, shipped })),
  };
}
export const RegistryContext = createContext<Registry | null>(null);
export function useRegistry() {
  const registry = useContext(RegistryContext);
  if (!registry) throw new Error("Feature composition is missing.");
  return registry;
}
