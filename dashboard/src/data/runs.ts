...
export class RunCoordinator {
  ...
  async submit(
    ...
    queryClient.setQueryData<Message[]>(keys.messages(sessionId), (old) => [
      ...(old ?? []),
      sending,
    ]);
    try {
      const run = await api<Run>("/submissions", {
        id,
        sessionId,
        text,
        replyTo,
      ...
      queryClient.setQueryData<Message[]>(keys.messages(sessionId), (old) =>
        old?.map((message) =>
          message.id === id ? { ...message, delivery: "unknown" } : message,
        ),
      );
    }
  }
  async resolveSubmission(id: string) {
    await this.accept(await api<Run>(`/submissions/${id}`));
  }
  private async accept(run: Run) {
    const snapshot = await api<{ run: Run; messages: Message[] }>(
      `/runs/${run.id}/snapshot`,
    );
    queryClient.setQueryData(keys.object("runs", run.id), snapshot.run);
    queryClient.setQueryData<Message[]>(keys.messages(run.sessionId), (old) =>
      snapshot.messages.map((message) => {
        const current = old?.find((item) => item.id === message.id);
        return current &&
          (current.streamCursor ?? 0) > (message.streamCursor ?? 0)
          ? current
          : message;
      }),
    );
  ...
  async stop(id: string) {
    ...
    queryClient.setQueryData(
      keys.messages(run.sessionId),
      await api(`/messages/${run.sessionId}`),
    );
  }
  private apply(event: StreamEvent) {
    if (event.id <= (this.cursors.get(event.runId) ?? 0)) return;
    this.cursors.set(event.runId, event.id);
    const run = queryClient.getQueryData<Run>(keys.object("runs", event.runId));
    if (!run) return;
    queryClient.setQueryData<Run>(keys.object("runs", event.runId), {
      ...run,
      cursor: event.id,
      status: event.type === "complete" ? "completed" : "streaming",
    });
    if (event.type === "delta")
      queryClient.setQueryData<Message[]>(keys.messages(run.sessionId), (old) =>
        old?.map((message) =>
          message.id === run.messageId && event.id > (message.streamCursor ?? 0)
            ? {
                ...message,
  ...
  private async subscribe(id: string) {
    ...
    try {
      const response = await request(
        `/runs/${id}/events?after=${this.cursors.get(id) ?? 0}`,
      ...
      /* A missing stream receipt is preserved below, never inferred as stopped. */
    } finally {
      this.subscriptions.delete(id);
      ...
      if (
        ...
        queryClient.setQueryData(keys.object("runs", id), {
          ...run,
          status: "interrupted",
        });
    }
