...
function Navigation({ close }: { close?: () => void }) {
  ...
  const { signOut } = usePreview();
  const [menu, setMenu] = useState(false);
  const profile = useObject<{ name: string }>("profile", "owner");
            ...
            <Popover.Trigger asChild>
              ...
              >
                ...
                <span className="owner-text">
                  <strong>{profile.data?.name ?? "Your space"}</strong>
                  <small>On your own terms</small>
                </span>
                <ChevronUp />
              </Button>
            </Popover.Trigger>
  ...
  if (signedOut)
    return (
      <div className="setup-layout">
        <h1>You’re signed out of the preview.</h1>
        <p>
          No authenticated session exists here. Fixture records remain in this
          tab.
        </p>
        <Button onClick={signIn}>Return to preview</Button>
      </div>
    );
  return (
        ...
        <div className="topbar">
          <div className="topbar-left">
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              <SheetContent side="left" className="mobile-sidebar">
                <SheetHeader className="sr-only">
                  <SheetTitle>Conker navigation</SheetTitle>
                  <SheetDescription>
                    Your daily loop, reference and controls.
                  </SheetDescription>
                </SheetHeader>
            ...
            <span className="crumb">
              ...
              <strong>
                {messenger ? "Conversations" : location.pathname.split("/")[1]}
              </strong>
            </span>
          </div>
          <Badge variant="outline">Fixture preview</Badge>
        </div>
        <main
          id="main"
          className={messenger ? "messenger-main" : "screen-main"}
        >
          <Outlet />
        </main>
      </div>
