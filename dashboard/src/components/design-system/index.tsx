...
export function CollectionSearch({ label, ...props }: Omit<ComponentProps<typeof Input>, "className" | "type" | "aria-label"> & { label: string }) {
  return <div data-slot="collection-search" className="relative w-full min-w-0">
    ...
    <Input {...props} type="search" aria-label={label} autoComplete="off" className="h-(--collection-search-height) rounded-xl bg-muted/20 pr-4 pl-11 shadow-none" />
  </div>
}
...
export function CollectionRow({ to, title, description, descriptionTitle, leading, trailing }: {
  ...
  return <li>
    <Link data-slot="collection-row" to={to} aria-labelledby={id} aria-describedby={`${id}-preview${trailing ? ` ${id}-meta` : ""}`}
      className="group flex min-h-(--collection-row-height) min-w-0 items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-muted/50 focus-visible:bg-muted/50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring sm:px-4">
...
export function CollectionSection({ title, icon, children }: { title: string; icon?: ReactNode; children: ReactNode }) {
  const id = useId()
  return <section aria-labelledby={id} data-slot="collection-section">
    <h2 id={id} className="mb-1 flex items-center gap-2 px-3 text-xs font-medium text-muted-foreground sm:px-4">{icon}{title}</h2>
