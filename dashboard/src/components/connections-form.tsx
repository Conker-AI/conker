...
export function ConnectionsFields({ value, onChange }: { value: Connections; onChange: (value: Connections) => void }) {
  ...
  return <div className="space-y-6">
    ...
    {serviceNames.map(name => <fieldset key={name} className="min-w-0 space-y-4 border-t pt-4">
      <legend className="pr-3 text-base font-medium">{labels[name]}</legend>
      <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm leading-6 text-muted-foreground">{descriptions[name]}</p><StatusBadge>Not connected</StatusBadge></div>
