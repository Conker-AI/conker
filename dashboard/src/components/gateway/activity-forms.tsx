...
export function GatewayTaskEditor({ draft, task, sessions, tasks, runs, referencesPending, referencesError, moreRuns, onMoreRuns, onRetryRuns, onChange, onSave, onClose, onDiscard, disabled, error }: {
  ...
  return <form className="flex min-h-0 flex-1 flex-col overflow-hidden" onSubmit={event => { event.preventDefault(); const invalid = validateTaskDraft(draft); setValidation(invalid); if (!invalid && !disabled && !stale) onSave() }}>
    <OverlayBody><fieldset disabled={disabled} className="min-w-0 space-y-5">
      <p className="text-xs leading-5 text-muted-foreground">This records an outcome for Companion. It does not start execution or change permissions.</p>
      ...
      <div className="space-y-2"><Label htmlFor={`${id}-session`}>Source conversation</Label>{task ? <p className="break-words text-sm">{source?.title || task.sessionId}<span className="mt-1 block text-xs text-muted-foreground">The source conversation stays fixed after creation.</span></p> : <SearchableSelect id={`${id}-session`} label="Source conversations" disabled={disabled} value={draft.sessionId} onChange={value => update({ sessionId: value, parentTaskId: '', runIds: [] })} options={sessions.filter(session => session.status === 'open').map(session => ({ value: session.id, label: session.title || session.id }))} />}</div>
      <div className="space-y-2"><Label htmlFor={`${id}-parent`}>Parent task</Label><SearchableSelect id={`${id}-parent`} label="Parent tasks" disabled={disabled || !draft.sessionId} value={draft.parentTaskId || 'none'} onChange={value => update({ parentTaskId: value === 'none' ? '' : value })} options={[{ value: 'none', label: 'No parent task' }, ...parents.map(parent => ({ value: parent.id, label: taskTitle(parent) })), ...(missingParent ? [{ value: draft.parentTaskId, label: `Current link · ${draft.parentTaskId} (not eligible or not loaded)` }] : [])]} /><p className="text-xs text-muted-foreground">Choose an active task in this conversation from the loaded records. Load more tasks from the list if needed.</p></div>
      <fieldset className="space-y-3"><legend className="mb-2 text-sm font-medium">Link existing runs</legend><p className="text-xs text-muted-foreground">Only recorded turns from this conversation can be linked. Linking does not start or resume a turn.</p><CollectionSearch label="Search runs to link" placeholder="Search run IDs, status or model…" disabled={!draft.sessionId} value={runQuery} onChange={event => setRunQuery(event.target.value)} />
        {referencesPending && <p role="status" className="text-xs text-muted-foreground">Loading conversation runs…</p>}{referencesError && <div className="space-y-2"><p role="alert" className="text-sm text-destructive">{referencesError}</p><Button type="button" size="sm" variant="outline" disabled={referencesPending} onClick={onRetryRuns}>Retry loading runs</Button></div>}
        <div className="max-h-52 space-y-3 overflow-y-auto rounded-lg border p-3">{matchingRuns.map(run => <Label key={run.id} className="flex items-start gap-3 text-xs leading-5"><Checkbox checked={draft.runIds.includes(run.id)} onCheckedChange={checked => update({ runIds: checked ? [...draft.runIds, run.id] : draft.runIds.filter(value => value !== run.id) })} /><span className="min-w-0 break-all">{run.id}<span className="block text-muted-foreground">{run.status.replaceAll('_', ' ')}{run.model ? ` · ${run.model}` : ''}</span></span></Label>)}{!draft.sessionId ? <p className="text-xs text-muted-foreground">Choose a source conversation to load its runs.</p> : !matchingRuns.length && !referencesPending && !referencesError && <p className="text-xs text-muted-foreground">No matching runs in the loaded records.</p>}{missingRuns.map(runId => <div key={runId} className="space-y-1"><p className="break-all text-xs text-muted-foreground">{referencesPending ? 'Loading linked run' : 'Linked run not loaded or unavailable'}: {runId}</p><Button type="button" size="sm" variant="ghost" onClick={() => update({ runIds: draft.runIds.filter(value => value !== runId) })}>Remove link</Button></div>)}</div>{moreRuns && <Button type="button" size="sm" variant="outline" disabled={referencesPending} onClick={onMoreRuns}>Load more runs</Button>}
      </fieldset>
      ...
      {stale && <p role="alert" className="text-sm text-destructive">The task changed since this draft opened. Your draft is retained. Review the current record, then discard this draft before editing its new revision.</p>}
...
export function GatewayTaskReview({ task, draft, onChange, onSave, onClose, onDiscard, disabled, error }: {
  ...
  return <form className="flex min-h-0 flex-1 flex-col overflow-hidden" onSubmit={event => { event.preventDefault(); if (valid && !disabled && !stale) onSave() }}><OverlayBody><fieldset disabled={disabled} className="space-y-5">
    <p className="break-words text-sm font-medium">{taskTitle(task)}</p><p className="text-sm leading-6 text-muted-foreground">Record your review as {taskStatusLabels[draft.status].toLocaleLowerCase()}. This reports task progress; it does not start, stop or resume linked turns.</p>
