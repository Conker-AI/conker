...
type Props = { client: GatewayActivityClient; runtime: GatewayRuntimeClient; state: GatewayActivityWorkspaceState; sourcePrivacy: GatewaySourcePrivacyState; active: boolean }
const uniqueRows = <T extends { id: string }>(old: T[], incoming: T[]) => [...new Map([...old, ...incoming].map(item => [item.id, item])).values()]

export function GatewayActivityWorkspace({ client, runtime, state, sourcePrivacy, active }: Props) {
  ...
  const epoch = retained.epoch, busy = retained.mutation?.phase === 'pending'
  ...
  async function mutate(operation: ActivityMutation, action: (signal: AbortSignal) => Promise<GatewayTask>) {
    ...
    try {
      ...
      state.setState(value => { const drafts = { ...value.drafts }, reviews = { ...value.reviews }; delete drafts[operation.key]; delete reviews[operation.key]; return { mutation: null, dialog: null, drafts, reviews, notice: 'Task record saved. No execution was started.' } })
  ...
  async function reconcile() {
    ...
    try {
      ...
      if (mutation.kind === 'create') {
        state.setState(value => { const drafts = { ...value.drafts }; delete drafts[mutation.key]; return { mutation: null, dialog: null, drafts, error: null, notice: 'The request identity resolved to a saved task. No creation was repeated.' } })
      } else state.setState({ mutation: { ...mutation, checked: true }, error: null, notice: 'Current task loaded. Review it before discarding the retained change; no write was repeated.' })
      ...
      state.setState({ ...(mutation.requestId && safe.status === 404 ? { mutation: { ...state.getState().mutation!, notFound: true } } : {}), error: mutation.requestId && safe.status === 404 ? 'No task was found for this request identity yet. You can check again or explicitly retry the same saved request. Its identity prevents duplicate creation.' : safe.message })
  ...
  function acceptCurrentRecord() {
    ...
    state.setState(value => { const drafts = { ...value.drafts }, reviews = { ...value.reviews }; delete drafts[mutation.key]; delete reviews[mutation.key]; return { mutation: null, dialog: null, drafts, reviews, error: null, notice: 'Current server record kept. Retained change discarded; no write was sent.' } })
  }
  ...
  const recovery = retained.mutation && retained.mutation.phase !== 'pending' ? <div className="space-y-3 rounded-lg border p-4"><p className="text-sm font-medium">{retained.mutation.phase === 'conflict' ? 'Server could not accept this change' : 'Task change outcome unknown'}</p><p className="text-xs leading-5 text-muted-foreground">Further task writes are blocked until the current record is checked. No operation will be repeated automatically. Available drafts are retained.</p>{retained.mutation.requestId && <p className="break-all text-xs text-muted-foreground">Request {retained.mutation.requestId}</p>}<div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" disabled={selectionPending} onClick={() => void reconcile()}>Check server record</Button>{retained.mutation.kind === 'create' && retained.mutation.phase === 'unknown' && retained.mutation.notFound && retained.mutation.input && !forgottenIds.includes(retained.mutation.input.sessionId) && <Button variant="outline" size="sm" disabled={selectionPending} onClick={retryCreation}>Retry same creation request</Button>}{retained.mutation.checked && <><Button variant="outline" size="sm" onClick={() => state.setState({ dialog: null })}>View current record</Button><Button variant="outline" size="sm" className="h-auto min-h-(--control-height-sm) whitespace-normal" onClick={acceptCurrentRecord}>Keep current record and discard change</Button></>}</div></div> : null
  ...
  return <section className="h-full min-h-0 overflow-y-auto p-4 sm:p-6 lg:p-8" tabIndex={0} aria-label="Activity workspace"><div className="space-y-5">
    <PageHeader density="compact" title="Activity" description="Track outcomes and inspect recorded attempts and events." actions={<><Button variant="outline" disabled={loading} onClick={() => void load()}><RefreshCw />Refresh</Button><Button disabled={!!retained.mutation} onClick={openCreate}><Plus />New task</Button></>} />
    <p className="text-xs leading-5 text-muted-foreground">Tasks are durable, owner-reported tracking for Companion. This workspace has no task executor.</p>
    ...
    {(taskFilter || runFilter || sessionFilter) && <div className="flex flex-wrap items-center gap-3 text-xs"><span className="break-all">Filtered source: {taskFilter || runFilter || sessionFilter}</span><Button size="sm" variant="outline" onClick={() => setParams({ tab })}>Clear source filter</Button></div>}
    {loading && <p role="status" className="text-xs text-muted-foreground">Loading recorded {tab}…</p>}
    ...
    {tab === 'runs' && (runs.length ? <GatewayRunsTable runs={runs} onOpen={openRun} /> : !loading && <CollectionEmpty title="No recorded runs" description="Turns appear here when Pi supplies a persisted record. Creating a task does not create a run." action={<Button asChild variant="outline"><Link to="/chats">Open conversations</Link></Button>} />)}
    {tab === 'events' && (events.length ? <GatewayEventsTable events={events} onOpen={openEvent} /> : !loading && <CollectionEmpty title="No recorded events" description="Task changes and supplied runtime events appear here without authored content." />)}
    {cursor && <Button variant="outline" disabled={loading} onClick={() => void load(true, cursor)}>Load more {tab}</Button>}
    ...
    <DetailPanel open={active && !!(taskId || runId || eventId) && !dialog} onOpenChange={open => { if (!open) closeDetails() }} title={safeSelectedTask?.id === taskId ? taskTitle(safeSelectedTask) : taskId ? 'Recorded task' : runId ? 'Recorded run' : 'Recorded event'} description="Saved source record" busy={busy}>
      {selectionPending ? <OverlayBody><p role="status">Loading record…</p></OverlayBody> : selectionError ? <OverlayBody><p role="alert" className="text-sm text-destructive">{selectionError}</p><Button variant="outline" onClick={() => { if (taskId) void loadSelection(taskId, 'task'); else if (runId) void loadSelection(runId, 'run') }}>Retry read</Button></OverlayBody> : taskId && safeSelectedTask?.id === taskId ? <GatewayTaskDetails task={safeSelectedTask} feedback={<>{retained.error && <p role="alert" className="text-sm text-destructive">{retained.error}</p>}{recovery}</>} disabled={!!retained.mutation} onEdit={() => openEdit(safeSelectedTask)} onReview={status => openReview(safeSelectedTask, status)} onArchive={() => void mutate({ key: `archive:${safeSelectedTask.id}`, taskId: safeSelectedTask.id, revision: safeSelectedTask.revision, kind: 'archive', phase: 'pending' }, signal => client.archiveTask(safeSelectedTask.id, { expectedRevision: safeSelectedTask.revision, archived: !safeSelectedTask.archivedAt }, { signal }))} /> : runId && selectedRun?.id === runId ? <GatewayRunDetails run={selectedRun} /> : selectedEvent ? <GatewayEventDetails event={selectedEvent} /> : <OverlayBody><p className="text-sm text-muted-foreground">This record is not available in the loaded page. Load more or clear the selection.</p></OverlayBody>}
    </DetailPanel>
    ...
    <span className="sr-only">Workspace revision {epoch}</span>
  </section>
}
