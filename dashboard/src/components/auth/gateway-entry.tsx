...
const services = (() => {
  try {
    ...
    return { value: { ...value, ownerState, owner: createGatewayOwnerClient(value.auth), control: createGatewayControlClient(value.auth), workspace, activityState, sourcePrivacy, activity: createGatewayActivityClient(value.auth), runtime: createGatewayRuntimeClient(value.auth) }, error: null }
...
export default function GatewayEntry() {
  ...
  return <><GatewayBoundary store={services.value.store}><GatewayWorkspace owner={services.value.owner} ownerState={services.value.ownerState} control={services.value.control} authStore={services.value.store} runtime={services.value.runtime} activity={services.value.activity} conversationState={services.value.workspace} activityState={services.value.activityState} sourcePrivacy={services.value.sourcePrivacy} /></GatewayBoundary><GatewayOperationVerification store={services.value.verification} authStore={services.value.store} /></>
}
