...
function VerificationForm({ store }: { store: GatewayVerificationStore }) {
  ...
  return <form className="flex min-h-0 flex-col" onSubmit={event => {
    ...
    <div className="space-y-2"><Label htmlFor="gateway-operation-password">Owner password</Label><Input id="gateway-operation-password" name="password" type="password" autoComplete="current-password" maxLength={2048} required autoFocus disabled={pending} value={password} onChange={event => setPassword(event.target.value)} /><p className="text-xs text-muted-foreground">Spaces and Unicode are preserved. The password is cleared from this form before verification.</p></div>
...
export function GatewayOperationVerification({ store, authStore }: { store: GatewayVerificationStore; authStore: GatewayAuthStore }) {
  ...
  return <Dialog open={visible && !!challenge} onOpenChange={open => { if (!open) store.getState().cancel() }}><TaskDialogContent title={challenge?.title ?? 'Verify operation'} description="Confirm the target, then verify your owner password." onInteractOutside={event => event.preventDefault()}>
