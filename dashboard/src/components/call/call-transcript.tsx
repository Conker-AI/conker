...
export function CallTranscript({ call, progress, speechEventId }: { call: CallSession; progress?: SpeechProgress | null; speechEventId?: string | null }) {
  ...
  return <div role="log" aria-label="Call conversation" aria-live="polite" tabIndex={0} className="call-transcript min-h-0 flex-1 overflow-y-auto p-5 outline-offset-[-4px] sm:p-6">
    ...
    <div className="mx-auto max-w-2xl space-y-5">
      ...
      {messages.map(item => <article key={item.id} className={item.kind === "user" ? "ml-auto max-w-[85%]" : "mr-4"}>
        ...
        {item.kind === "assistant" && speechEventId === item.id && progress?.text === item.text ? <CallSpeechText text={item.text} progress={progress} compact /> : <p dir="auto" className={item.kind === "user" ? "whitespace-pre-wrap break-words rounded-lg bg-muted px-4 py-3 text-sm leading-6" : "whitespace-pre-wrap break-words text-sm leading-6"}>{item.text}</p>}
      </article>)}
      {busy && <p role="status" className="flex items-center gap-2 text-xs text-muted-foreground"><span className={call.paused ? "size-1.5 rounded-full bg-muted-foreground" : "size-1.5 rounded-full bg-primary motion-safe:animate-pulse"} />{call.paused ? "Response paused" : call.phase === "thinking" ? "Thinking…" : "Writing…"}</p>}
    </div>
    <div ref={tail} />
  </div>
}
...
export function CallComposer({ call }: { call: CallSession }) {
  ...
  return <form className="shrink-0 border-t border-border bg-background p-3" aria-label="Call message" onSubmit={event => { event.preventDefault(); void send() }}>
    <div className="flex items-end gap-2 rounded-lg border border-input bg-card p-2 focus-within:border-ring">
      ...
      {busy ? <Button type="button" variant="outline" size="icon" aria-label="Interrupt response" onClick={interrupt}><Square /></Button> : <Button size="icon" type="submit" aria-label="Send call message" disabled={call.paused || !draft.trim() || draft.length > 4000}><ArrowUp /></Button>}
    </div>
    <p className="mt-1.5 text-xs text-muted-foreground">{call.paused ? "Draft kept · resume the call to send" : "Enter to send · Shift + Enter for a new line"}</p>
  </form>
}
