  ...
  return <section aria-label="Artifact canvas" className={cn("flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden", className)}>
    ...
    <Tabs value={mode} onValueChange={setMode} className="flex min-h-0 flex-1 flex-col gap-0">
      ...
      <TabsContent value="source" className="min-h-0 flex-1 space-y-4 overflow-auto p-4 sm:p-6">
        ...
        <div className="space-y-2"><Label htmlFor={`artifact-source-${artifact.id}`}>{selected.content.kind !== "markdown" && selected.content.kind !== "code" ? "Structured data (JSON)" : "Source text"}</Label><Textarea id={`artifact-source-${artifact.id}`} value={sourceValue} onChange={event => patch({ source: event.target.value })} readOnly={!editable} disabled={busy} rows={16} maxLength={250000} spellCheck={selected.content.kind === "markdown"} className={selected.content.kind === "markdown" ? "" : "font-mono text-xs"} /></div>
        {(selected.content.kind !== "markdown" && selected.content.kind !== "code") && <p className="text-xs text-muted-foreground">{selected.content.kind === "table" ? "Columns are labels; rows contain one text cell per column. Limit: 32 columns and 1,000 rows." : selected.content.kind === "diagram" ? "Use nodes with unique id, label, x and y, plus optional description; edges need unique id, source and target node IDs, plus optional label. Limit: 100 nodes, 200 connections; x/y from -10,000 to 10,000. No self-connections or executable fields." : selected.content.kind === "media" ? "Use mediaType image, audio or video, an HTTPS url and a description. External media loads only when requested in Preview; versions and exports save the reference, not the remote file." : "Choose bar, line or area. Each row has a label and one finite numeric value per series. Limit: 8 series and 500 rows. No expressions or styling code."}</p>}
