...
function ArtifactDetails({ artifact, open, onClose }: { artifact: ArtifactView; open: boolean; onClose: () => void }) {
  ...
  return <DetailPanel open={open} onOpenChange={value => { if (!value) onClose() }} title="Artifact details" description="Origin, version history and source privacy">
    <OverlayBody><ReferenceSection title="Origin"><p className="text-sm">{artifact.origin === "owner-authored" ? "Created by the owner." : "Explicit copy of a completed assistant response. Editing this artifact never rewrites that response."}</p>{sourceHref && <Button asChild variant="outline"><Link to={sourceHref}>Open original response</Link></Button>}</ReferenceSection>
      ...
      <ReferenceSection title="Record"><p className="text-sm">{artifact.versionCount} immutable versions. Revision {artifact.revision}.</p><p className="text-xs text-muted-foreground">Local preview; changes reset on reload. Code remains inert. HTML apps run only in an explicitly started sandbox preview.</p></ReferenceSection>
    </OverlayBody>
  </DetailPanel>
}
...
function ReadableCanvas({ artifact, client, initialVersion, onVersionChange, headerActions, headingRef, onClose, readOnly = false, className }: ArtifactCanvasProps) {
  ...
  return <section aria-label="Artifact canvas" className={cn("flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden", className)}>
    ...
    <Tabs value={mode} onValueChange={setMode} className="flex min-h-0 flex-1 flex-col gap-0">
      ...
      <TabsContent value="preview" className="min-h-0 flex-1 overflow-auto p-4 sm:p-6" tabIndex={0} aria-label="Artifact preview content">{parsed.error ? <p role="alert" className="text-sm text-destructive">{parsed.error} Open Source to correct the draft.</p> : parsed.content && <ArtifactPreview key={`${artifact.id}:${selected.version}`} content={parsed.content} citations={selected.citations} />}</TabsContent>
      <TabsContent value="source" className="min-h-0 flex-1 space-y-4 overflow-auto p-4 sm:p-6">
        ...
        <div className="space-y-2"><Label htmlFor={`artifact-source-${artifact.id}`}>{selected.content.kind !== "markdown" && selected.content.kind !== "code" && selected.content.kind !== "html" ? "Structured data (JSON)" : "Source text"}</Label><Textarea id={`artifact-source-${artifact.id}`} value={sourceValue} onChange={event => patch({ source: event.target.value })} readOnly={!editable} disabled={busy} rows={16} maxLength={250000} spellCheck={selected.content.kind === "markdown"} className={selected.content.kind === "markdown" ? "" : "font-mono text-xs"} /></div>
        {(selected.content.kind !== "markdown" && selected.content.kind !== "code" && selected.content.kind !== "html") && <p className="text-xs text-muted-foreground">{selected.content.kind === "table" ? "Columns are labels; rows contain one text cell per column. Limit: 32 columns and 1,000 rows." : selected.content.kind === "diagram" ? "Use nodes with unique id, label, x and y, plus optional description; edges need unique id, source and target node IDs, plus optional label. Limit: 100 nodes, 200 connections; x/y from -10,000 to 10,000. No self-connections or executable fields." : selected.content.kind === "media" ? "Use mediaType image, audio or video, an HTTPS url and a description. External media loads only when requested in Preview; versions and exports save the reference, not the remote file." : "Choose bar, line or area. Each row has a label and one finite numeric value per series. Limit: 8 series and 500 rows. No expressions or styling code."}</p>}
        ...
        {selected.content.kind === "html" && <p className="text-xs text-muted-foreground">Up to 200,000 characters of HTML with inline CSS and JavaScript. Run it explicitly in Preview. External libraries, network resources and Conker tools are unavailable. Export saves inert .html.txt source.</p>}{parsed.error && <p role="alert" className="text-sm text-destructive">{parsed.error}</p>}
      </TabsContent>
